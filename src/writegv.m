function writegv(fdot, spar, nd, el, bc, src)
%writegv(fdot, spar, nd, el, bc, src)
%
%  Description:
%
%    Write a GraphViz DOT file of the model and results.
%    See http://www.graphviz.org/ for the rendering software.
%
%==========================================================================
...
ncolors = 256;  %  Number of colors in color map
maxT = max([nd(:).T]);
minT = min([nd(:).T]);
Tspace = linspace(minT,maxT,ncolors);
maxQ = max([el(:).Q]);
minQ = min([el(:).Q]);
Qspace = linspace(minQ,maxQ,ncolors);

%  Diverging colormap
%  see http://www.sandia.gov/~kmorel/documents/ColorMaps/

RGBbeg = [0.230, 0.299, 0.754];  %  Cool
RGBend = [0.706, 0.016, 0.150];  %  Warm
space  = linspace(0,1,ncolors);
cmap   = diverging_map(space, RGBbeg, RGBend);
hcmap  = rgb2hex(cmap);

fprintf(fdot,'digraph TNSolver {\nsize = "6, 3";\n');
fprintf(fdot,'orientation=portrait;\n');
fprintf(fdot,'rankdir=LR;\n');
fprintf(fdot,'/*orientation=landscape;*/\n');
fprintf(fdot,'/*rankdir=BT;*/\n');
fprintf(fdot,'graph [fontname="Helvetica", fontsize=20.0];\n');
fprintf(fdot,'node [fontname="Helvetica", fontsize=20.0, style="filled"];\n');
fprintf(fdot,'edge [fontname="Helvetica", fontsize=20.0, penwidth=5];\n');

for e=1:nel
  ndx = find(Qspace <= el(e).Q, 1, 'last');
  fprintf(fdot,' %s -> %s [label = "%s\\n%s %s", color="%s"];\n', ...
          el(e).nd1,el(e).nd2,el(e).label,num2str(el(e).Q),u.Q,hcmap(ndx,:));
end

for n=1:nnd
  ndx = find(Tspace <= nd(n).T, 1, 'last');
  fprintf(fdot,' %s [label = "%s\\n%s %s", fillcolor = "%s"]\n',  ...
          nd(n).label,nd(n).label,num2str(nd(n).T),u.T,hcmap(ndx,:));
end  

for i=1:spar.nDBC
  for j=1:length(bc(spar.Dirichlet(i)).nds)
    fprintf(fdot,' %s [shape=triangle];\n', bc(spar.Dirichlet(i)).nds{j});
  end
end  
