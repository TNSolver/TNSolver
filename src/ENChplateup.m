function [h, Ra, Nu] = ENChplateup(mat, L, Ts, Tinf)
%[h, Ra, Nu] = ENChplateup(mat, L, Ts, Tinf)
%
%  Description:
%
...
if L <= 0.0
  error('ENChplateup: Invalid charcteristic length = %g\n',L)
end
...
if Pr < 0.7
    fprintf('WARNING: ENChplateup - Pr = %g, is below 0.7\n',Pr)
end
...
if Ts >= Tinf  %  Hot plate facing up
  ...
  if Ra < 1.0E4 || Ra > 1.0E11
    fprintf('WARNING: ENChplateup - Ra = %g, is out of range: 1.0E4 - 1.0e11\n',Ra)
  end
  ...
  if Ra < 1.0E4 || Ra > 1.0E9
    fprintf('WARNING: ENChplateup - Ra = %g, is out of range: 1.0E4 - 1.0e9\n',Ra)
  end
