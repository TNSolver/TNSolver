function [h, Ra, Nu] = ENCiplatedown(mat, H, L, theta, Ts, Tinf)
%[h, Ra, Nu] = ENCiplatedown(mat, H, L, theta, Ts, Tinf)
%
%  Description:
%
...
if H <= 0.0
  error('ENCiplatedown: Invalid charcteristic height, H = %g\n',H)
end
if L <= 0.0
  error('ENCiplatedown: Invalid charcteristic length, L = A/P = %g\n',L)
end
if theta < 0 || theta > 90
  error('ENCiplatedown: Invalid inclination angle = %g\n',theta)
end  
...
%--------------------------------------------------------------------------
if Ts < Tinf  %  Unstable - cold plate facing down
%--------------------------------------------------------------------------
...
%  Rayleigh number - horizontal plate
  ...
  if Rah < 1.0E4 || Rah > 1.0E11
    fprintf('WARNING: ENCiplatedown - Ra_h = %g, is out of range: 1.0E4-1.0e11\n',Rah)
  end
...
%--------------------------------------------------------------------------
else  %  Ts > Tinf  Stable - hot plate facing down
%--------------------------------------------------------------------------
